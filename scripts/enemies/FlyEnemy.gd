...
const FlyEnemyBullet = preload("res://scripts/projectiles/FlyEnemyBullet.gd")
...
func _die_fly_enemy():
	alive = false
	var game = get_tree().current_scene
	if game and game.has_method("_spawn_item_drop"):
		game._spawn_item_drop(global_position)
