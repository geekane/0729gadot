...
var spike_count = 0
var total_test_frames = 1200  # 增加到 1200 帧以覆盖 Boss 战场
...
func _initialize():
	print("============================================================")
	print("[TestRunner] 🔬 启动 1200 帧白闪诊断深度分析测试 (含 Boss 战场)")
...
func _on_process_frame():
	...
	# ─── 2. 游戏内模拟激进游玩与跨关加载 ──────────────
		...
		# 在 Frame 250 模拟通关第 1 关
		if frame_count == 250:
			print("[TestRunner] 模拟到达 Level 1 终点 -> 触发通关")
			game_instance._win_game()
		# 在 Frame 253 模拟按下空格键进入下一关（隔帧让待决 tween/timer 自然消亡）
		elif frame_count == 253:
			print("[TestRunner] 模拟按下空格键 -> 挑战 Level 2")
		...
		# 在 Frame 600 传送玩家到 Boss 战场入口，触发 Boss 区电影级过场
		elif frame_count == 600:
			print("[TestRunner] 🎯 传送至 Boss 战场入口 (x=4000)")
			game_instance.god_mode_enabled = true
			var p2 = game_instance.player_node
			if p2 and is_instance_valid(p2):
				p2.position = Vector2(4000, 500)
			if game_instance.camera and is_instance_valid(game_instance.camera):
				game_instance.camera.position = Vector2(4000, 500)
		# 在 Frame 660 检查是否成功触发 Boss 战场
		elif frame_count == 660:
			print("[TestRunner] 🎬 Boss 战场状态: locked=%s camera_fixed=%s" % [
				str(game_instance.boss_arena_locked),
				str(game_instance.camera_fixed_boss_arena)])
			var boss_portrait = game_instance.get_node_or_null("BossPortrait")
			var boss_overlay = game_instance.get_node_or_null("BossArenaOverlay")
			print("[TestRunner] 🖼️  BossPortrait exists=%s  BossArenaOverlay exists=%s" % [
				str(boss_portrait != null and is_instance_valid(boss_portrait)),
				str(boss_overlay != null and is_instance_valid(boss_overlay))])
			if boss_portrait and is_instance_valid(boss_portrait):
				print("[TestRunner]     Portrait alpha=%.3f modulate=%s" % [boss_portrait.modulate.a, str(boss_portrait.modulate)])
			_queue_capture("screenshot_boss_arena.png")
		...
		if p and is_instance_valid(p) and not p.is_dead:
			...
			# Boss 战场阶段 (frame >= 600): 持续向右接近 Boss
			if frame_count >= 620 and frame_count < 1100:
				p.velocity.x = p.SPEED * 1.0
				p.facing_right = true
				# 保持在地面附近
				if p.position.y > 510 and p.is_on_floor():
					p.velocity.y = p.JUMP_VELOCITY
			
			if frame_count % 25 == 0 and p.is_on_floor() and frame_count < 600:
				p.velocity.y = p.JUMP_VELOCITY
			...
			if frame_count % 20 == 0:
				...
				mb.pressed = true
				if frame_count >= 650:
					# Boss 战阶段：瞄准 Boss 位置 (x=4820, y=180)
					mb.position = Vector2(4820, 180)
				else:
	...
	if frame_count == 60:
		...
		_queue_capture("screenshot_boss_fight.png")
	elif frame_count == 1100:
		_queue_capture("screenshot_boss_endgame.png")
...
func _diagnose_ghost_frame(current_count: int):
	...
	if diff >= 2 and frame_count > 10:
		...
		# 在幽灵帧发生时自动截图取证 (跳过关卡过渡期的批量清理)
		if abs(frame_count - 250) > 10 and abs(frame_count - 500) > 10 and abs(frame_count - 600) > 10:
