...
const SPEED = 360.0
const MAX_RUN_SPEED = 620.0      # 极速冲刺速度 (按住方向键极速奔跑，翻滚疾风手感)
const RUN_ACCEL = 850.0           # 奔跑加速度 (/秒)
...
const Batarang = preload("res://scripts/projectiles/Batarang.gd")
...
func _physics_process(delta):
	...
	if currently_on_floor and not was_on_floor:
		if has_node("/root/SoundManager"):
			get_node("/root/SoundManager").call("play", "land", -5.0)
	...
	if jump_buffer_timer > 0 and coyote_timer > 0:
		if has_node("/root/SoundManager"):
			get_node("/root/SoundManager").call("play", "jump", -3.0, randf_range(0.95, 1.05))
			get_node("/root/SoundManager").call("play", "cape", -8.0)
...
func shoot_batarang():
	...
	_play_sound("batarang_throw", -5.0)
...
func melee_attack():
	...
	_play_sound("melee_swing", -3.0, randf_range(0.95, 1.05))

func _play_sound(sound_name: String, volume: float = 0.0, pitch: float = 1.0):
	if has_node("/root/SoundManager"):
		get_node("/root/SoundManager").call("play", sound_name, volume, pitch)
